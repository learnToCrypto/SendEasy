...
func (demand *Demand) NumReplies() (count int) {
	rows, err := Db.Query("SELECT count(*) FROM messages where demand_id = $1", demand.Id)
	if err != nil {
		return
	}
	for rows.Next() {
...
func (demand *Demand) Messages() (messages []Message, err error) {
	rows, err := Db.Query("SELECT id, uuid, body, user_id, demand_id, created_at FROM messages where demand_id = $1", demand.Id)
...
func (user *User) CreateDemand(object, collection, delivery, timeframe string, status int) (conv Demand, err error) {
	...
	err = stmt.QueryRow(createUUID(), object, collection, delivery, timeframe, user.Id, time.Now(), status).Scan(&conv.Id, &conv.Uuid, &conv.Object, &conv.Collection, &conv.Delivery, &conv.Timeframe, &conv.UserId, &conv.CreatedAt, &conv.Status)
	return
}
...
func (user *User) CreateMessage(conv Demand, body string) (msg Message, err error) {
	...
	defer stmt.Close()
	// use QueryRow to return a row and scan the returned id into the Session struct
	err = stmt.QueryRow(createUUID(), body, user.Id, conv.Id, time.Now()).Scan(&msg.Id, &msg.Uuid, &msg.Body, &msg.UserId, &msg.DemandId, &msg.CreatedAt)
	return
}

// Get all demands in the database and returns it
func Demands() (demands []Demand, err error) {
	rows, err := Db.Query("SELECT id, uuid, object, collection, delivery, timeframe, user_id, created_at, status FROM demands ORDER BY created_at DESC")
	if err != nil {
		return
	}
	for rows.Next() {
...
func DemandByUUID(uuid string) (conv Demand, err error) {
	conv = Demand{}
	err = Db.QueryRow("SELECT id, uuid, object, collection, delivery, timeframe, user_id, created_at, status FROM demands WHERE uuid = $1", uuid).
...
func (demand *Demand) User() (user User) {
	user = User{}
	Db.QueryRow("SELECT id, uuid, name, email, created_at FROM users WHERE id = $1", demand.UserId).
...
func (msg *Message) User() (user User) {
	user = User{}
	Db.QueryRow("SELECT id, uuid, name, email, created_at FROM users WHERE id = $1", msg.UserId).
		Scan(&user.Id, &user.Uuid, &user.Name, &user.Email, &user.CreatedAt)
	return
}
